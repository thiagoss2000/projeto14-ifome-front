...
export default function MainPage() {
  const URL = "https://projeto14.herokuapp.com"
  ...
  useEffect(() => {
    axios
      .get(`${URL}/products`, { headers: { user: user_id } })
      .then((response) => {
  ...
  function sendProducts() {
    ...
    axios
      .post(`${URL}/purchase`, selected, { headers: { user: user_id, token: token } })
      .then((response) => {
        setSelected([])
      })
      .catch((err) => {
    ...
    return (
      <Main>
        ...
        <div className="body" onClick={() => setUserMenu(false)}>
          ...
          <button onClick={sendProducts} className={itens.length !== 0 ? "" : "displayNone"}>
            go shopping
          </button>
          <div>
            {category.map((el) => {
